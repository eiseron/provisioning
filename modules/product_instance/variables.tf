...
variable "backup" {
  description = "DB backup wiring. When bucket_name is set the module creates three protected CI variables on the ops project (PROD_BACKUP_BUCKET, PROD_BACKUP_NAME, PROD_BACKUP_AGE_RECIPIENTS), two pipeline schedules (a daily staleness verifier and a weekly restore drill), and a Kubernetes CronJob in the product's own namespace running the backup itself (eiseron db backup: pg_dump scoped to the product's own tenant database, age-encrypted, uploaded to bucket_name). PROD_BACKUP_NAME is always var.slug (the R2 object-key prefix and the PGUSER/PROD_BACKUP_DATABASE default the backup gem falls back to; no reason for it to ever differ from the product's own slug). The drill schedule is activated only when drill_key is also set, and the key lands as a masked CI variable (PROD_BACKUP_DRILL_KEY). The CronJob's R2 write credentials are read (not created) from PROD_BACKUP_AWS_ACCESS_KEY_ID/SECRET_ACCESS_KEY on ops_project_id -- these must already exist, published by the product's own module.<slug>/db_backup_r2 (the older 'product' module) with enable_db_backup = true, before bucket_name is set here. Defaults leave no resources created."
  type = object({
...
variable "backup_schedule" {
  description = "Cron schedule (Kubernetes CronJob syntax, UTC) for the in-cluster backup job. Only used when backup.bucket_name is set."
  type        = string
  default     = "0 4 * * *"
}
...
variable "runtime" {
  description = "App runtime-service wiring. When enable is true (and prod is enabled) the module stands up the product's runtime service, reusing the generated SECRET_KEY_BASE and building DATABASE_URL from db_tenant_password. The module owns the connection to the runtime cluster, so the consumer passes only data (never a provider) and stays agnostic to the runtime implementation behind it. Dormant by default until the product cutover. The cluster bearer token is a separate sensitive variable (cluster_token), never part of this object, so it is not exposed in plan output. Fields: cluster_host and cluster_ca_cert are the runtime cluster API endpoint and base64 CA the module connects with; namespace is where the app runs; app_host is the public host routed to the service; extra_hosts are additional hosts the IngressRoute also matches (e.g. a verification subdomain); app_image is the image used only on create/recreate (the running image is owned by the deploy); app_image_repo is the same image's repository path, without registry host or tag, published as PROD_APP_IMAGE_REPO for the CI facade's deploy-time build target; observability_otlp_endpoint is where the app sends telemetry; admin_access_* wire the OIDC admin gate; node_selector labels select the node the app runs on; release_module is the app's base release module (e.g. Afinados) used to build the deploy-time init container command that migrates then seeds (bin/<slug> eval '<release_module>.Release.migrate' then Release.seed), matching the Kamal post-deploy hook; empty disables it; env_clear and env_secret are extra product-specific environment variables merged into the app container's plain env and its Secret respectively (e.g. object-store and media settings such as MEDIA_*), so the consumer can supply variables the module does not model as first-class fields without widening the object per product; keys override the module defaults on collision."
  type = object({
    ...
    app_image_repo              = optional(string, "")
    observability_otlp_endpoint = optional(string, "http://observability-collector:4318")
    admin_access_issuer         = optional(string, "")
    admin_access_certs_url      = optional(string, "")
    admin_access_audiences      = optional(string, "")
    node_selector               = optional(map(string), {})
    release_module              = optional(string, "")
    env_clear                   = optional(map(string), {})
    env_secret                  = optional(map(string), {})
  })
  default = {}
