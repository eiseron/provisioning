...
stable. Run it in a **different location/network** than the production host so a
stolen production disk does not sit next to the key server; the disk-theft
protection holds regardless, since unlock is network-bound (McCallum-Relyea —
Tang never sees the key).
